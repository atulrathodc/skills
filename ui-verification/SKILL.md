...
3. **Check the console/network errors** — if the framework prints them to the served log or you can run a headless browser (`browser` tool / gstack / puppeteer), capture: JS errors (blank page, "X is not defined"), failed API calls (404/500), and CORS errors.
4. **Exercise the feature end-to-end** — the UI calls the backend; confirm the round-trip works through the real UI path. Drive the ACTUAL user interaction in the browser: fill the form, click the button, see the result render (see `frontend-backend-integration`, `http-api-testing`, `runtime-verification`).
...
6. Only `done()` when the page loads without errors AND the requested feature is usable through the UI — proven in a real browser, not just "the frontend serves".
